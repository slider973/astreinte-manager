...
|---|---|---|---|
| Troisième compteur du récapitulatif (§ 6.2) | « weekends » — unités de weekend attribuées | **« astreintes »** — le nombre d'attributions | « 62 créneaux · 18 pompiers · 4 weekends » mélange trois grandeurs sans rapport ; « 62 créneaux · 18 pompiers · 70 astreintes » se lit d'un trait. Le weekend est un quota de membre, pas une mesure d'expédition. |
...
| Le nom d'un membre dans le suivi (§ 6.4) | joint depuis `profiles` dans la même requête que les attributions | **lu dans `memberships`**, une requête de plus par mois | `display_name` — le nom d'usage de la caserne — est une colonne de `memberships`, pas de `profiles` (`docs/SCHEMA.md § 2.3`) : aucune jointure depuis `assignments` ne peut le rapporter. Vu en vrai contre PostgREST, qui répond `column profiles_1.display_name does not exist`. La lecture rapporte aussi les **membres désactivés depuis** : une attribution ne perd pas son nom parce que son titulaire est parti. |
...
| Retardataires : « relancé il y a 20 min » (§ 6.6) | sur chaque ligne | livré, et **le bouton reste actif** | Désactiver le bouton pendant une heure cacherait la raison. Le dédoublonnage est côté base ; l'écran dit ce qui s'est passé au lieu d'interdire le geste — « Ces pompiers ont déjà été relancés dans l'heure. » |
| `schedules` en temps réel (§ 6.8) | deux tables, un canal | **un seul canal Supabase**, deux souscriptions `postgres_changes` | Un canal par table doublait le coût de connexion pour la même information. Le tri par caserne reste côté client, comme au 017. Conséquence assumée : **le canal ne diffuse aucune jointure**, donc une réponse arrive sans le nom du membre — le contrôleur garde celui qu'il connaît déjà. |
| L'avancement en temps réel | non traité | **`v_schedule_progress` est relue**, 400 ms après la dernière réponse reçue | Le canal diffuse des lignes, pas des agrégats, et le brief interdit de recompter en Dart ce que la vue rend. Relire à chaque événement coûterait trente requêtes pour six nombres ; on attend que la salve se taise. |
| `IndicateurDirect` | non traité — le 017 l'avait en privé dans sa barre de commande | **extrait** dans `widgets/indicateur_direct.dart`, partagé par les deux écrans | Deux écrans qui affichent l'état du même canal ne doivent pas pouvoir en dire deux choses différentes. |
| Squelette du suivi (§ 5.3) | « un bloc de progression, trois lignes de créneaux » | le même, dans un `SingleChildScrollView` **inerte** | Un squelette plus haut que la fenêtre débordait de 176 px sur un poste de 900. Il se coupe désormais proprement, et il ne se défile pas : il n'y a rien à lire dessous. |
...
### Ce que la revue a rouvert, et ce que ça a changé

| Point | Ce que disait ce brief | Ce que fait le code | Pourquoi |
|---|---|---|---|
| Garder la machine à états (§ 7.1) | plannings : politique **et** déclencheur, cascade comprise | **les créneaux aussi** — `shifts_guard_suppression` et sa politique | Le brief n'avait vu qu'une moitié du chemin. `shifts` cascade vers `assignments` : supprimer **un créneau** d'un planning publié effaçait les attributions déjà reçues, en une écriture, sans trace. Reproduit en revue. La règle « rien n'est jamais supprimé » se garde à chaque niveau de cascade, pas au premier. |
| Effectif requis d'un créneau publié | non traité | `shifts_effectif_revalide` réévalue la complétude **dans les deux sens** | Un effectif qui change, c'est la condition de validation qui bouge sans qu'aucune réponse ne l'ait fait. 2 → 1 après une seule acceptation rend le planning complet ; 1 → 2 sur un planning validé le rend incomplet, et le laisser « validé » mentirait à toute la caserne (`docs/WORKFLOWS.md § 2`). |
| Validation « une seule fois » (§ 7.2) | l'`update … where status = 'published' returning` arbitre la course | **le verrou de ligne passe devant le test**, l'arbitrage reste en seconde sécurité | L'arbitrage ne servait à rien : chaque transaction lit son propre instantané, aucune ne voit l'acceptation de l'autre, donc **aucune n'atteint l'arbitrage**. Deux acceptations simultanées sur un créneau qui en demande deux laissaient le planning publié pour toujours. `scripts/test_concurrence.sh` le reproduit à deux sessions réelles : rouge avec l'ancien ordre, vert avec le verrou. |
| Complétude testée à l'acceptation | « la validation survient même si personne n'a l'application ouverte » (§ 2) | `schedule_complet` est **extraite** et appelée aussi en fin de `publish_schedule` | Un planning dont tous les créneaux demandent zéro personne est complet **avant** la moindre réponse, et le déclencheur n'est réveillé que par une réponse. Le § 5.1 de ce brief nommait le piège (« un planning à 62 créneaux et 0 requis partout est validé dès sa publication ») et le code ne le tenait pas. |
| Clé de dédoublonnage de `schedule_validated` | « un planning revalidé est un second fait, et il doit repartir » | **retirée** | La clé portait la seconde : deux validations successives dans la même seconde n'envoyaient qu'une notification — mesuré. L'exclusivité est déjà tenue par la transition ; la clé n'ajoutait rien et avalait un fait. |
| `notification.ok` (§ 5.2) | « bannière `erreur` en tête de feuille » pour un **échec de publication** | l'**échec d'envoi**, distinct, a sa phrase et son bandeau persistant sur le suivi | Le brief avait traité l'échec de publication et oublié le cas où la publication réussit mais l'envoi non. Le chef lisait « 18 pompiers notifiés » quand aucun téléphone n'avait sonné, et n'avait donc aucune raison d'aller relancer. Le bandeau reste jusqu'au rattrapage, et il ne se ferme pas : il décrit un état persistant (`DESIGN.md § AppBanner`). |
| Relance (§ 6.6) | un seul geste, sur les retardataires | **deux portées** : les retardataires (défaut) et **le rattrapage** (`p_tout`) | Les pompiers qui n'ont rien reçu à la publication ne sont pas « en retard » — ils viennent d'être proposés. Leur faire attendre `late_report_hours` avant de pouvoir les prévenir serait absurde. |
| Marque de la relance | « `reminder_count` mis à jour pour les attributions réellement relancées » | la marque est posée **après** avoir constaté que l'envoi est réel | Elle était posée avant. Un double clic faisait franchir un palier sans qu'aucune notification ne parte, et les crons du ticket 022 sautent le push de 24 h quand `reminder_count` vaut déjà 1 : le pompier perdait sa relance automatique. Un verrou consultatif sérialise les clics simultanés. |
| Compte des retardataires (§ 6.6) | non tranché — le brief nomme la vue au § 2 mais laisse la liste au client | **la base compte, l'écran nomme** : le titre lit `v_schedule_progress.assignments_late` | Deux définitions du retard, c'était deux chiffres pour une chose. La liste reste résolue côté client, parce que la vue ne rend qu'un nombre — et la relance vise de toute façon ce que la base voit, pas ce que la liste affiche. |
