...
**Et il ne suffit pas de garder les astreintes.** Constaté dans Chrome, API coupée : un démarrage
à froid n'atteignait pas cet écran du tout. La session se restaure toute seule depuis le stockage
local, mais la lecture de `memberships` échoue, `etatAuthProvider` reste en chargement et
l'application s'arrête sur « Pas de connexion ». **La caserne du membre est donc gardée elle
aussi**, sous la même mécanique (`core/session/appartenances_locales.dart`), et c'est elle qui rend
vrai le critère du ticket sur un téléphone. Le détail et ses conséquences sont au § 11.
...
> Rempli par `flutter-dev` après le code, et **après l'avoir regardé dans Chrome** sur la pile
> locale : planning d'octobre créé, publié, accepté depuis l'écran des propositions, puis validé.
> Ce qui est ici est la référence, pas ce qui précède. Les six premiers sont des défauts trouvés à
> l'écran, pas sur le papier.

| Point | Ce que disait ce brief | Ce que fait le code | Pourquoi |
|---|---|---|---|
| **Le démarrage à froid sans réseau** | « le cache est la source de vérité de l'affichage » — l'écran était censé suffire | **il ne suffisait pas** : l'application s'arrêtait sur « Pas de connexion » avant d'atteindre le moindre écran. Ajout de `core/session/appartenances_locales.dart` — la caserne du membre est gardée sur l'appareil, et `appartenancesProvider` y retombe quand `memberships` échoue | La session se restaure toute seule depuis le stockage local ; la **caserne**, non. Sans elle, `etatAuthProvider` reste en chargement et le cache d'astreintes ne sert jamais dans la seule scène qui le justifie — une remise sans couverture. Vu dans Chrome, API coupée, avant et après. C'est la pièce qui rend vrai le critère du ticket. |
| Le rôle gardé sur l'appareil | non traité | **tout ce qui revient du stockage est un simple membre** : le rôle n'est même pas écrit dans le document | Un rôle qu'on n'a pas pu revérifier n'accorde aucun privilège — c'est la règle déjà écrite dans `RoleMembre.depuisSql`, appliquée à une valeur qu'on ne peut pas confirmer plutôt qu'à une valeur qu'on ne comprend pas. Conséquence assumée : un chef de centre hors ligne perd l'onglet « Admin », qui ne lui ouvrirait que des listes vides et des formulaires qui échouent (`DESIGN.md § Écarts, ticket 024`). Il revient avec le réseau. |
| Le rafraîchissement | « à l'ouverture, au tirage, au retour au premier plan » | **à chaque ouverture de l'écran**, et pas seulement quand l'instantané vient du cache | Le contrôleur n'est pas auto-disposé : une proposition acceptée sur l'onglet voisin ne le réveillait jamais. Accepter puis passer sur « Astreintes » ne montrait rien — exactement le critère laissé ouvert par le ticket 021. Trouvé dans Chrome. |
| Mois d'ouverture du calendrier | non traité : implicitement le mois courant | **le mois de la prochaine astreinte**, et le mois courant à défaut | Un planning se construit un mois à l'avance : le mois courant est très souvent vide, et ouvrir sur une grille sans aucune marque demande de comprendre qu'il faut appuyer sur une flèche. Vu à l'écran : septembre vide alors que les trois astreintes étaient en octobre. |
| Largeur de la bascule et du calendrier | « colonne bornée à 720 » énoncé pour la liste seule | la bascule **et** la grille du calendrier sont bornées à 720 et centrées, et la bascule reprend la **marge de page** de la liste | Sur un portable de 1 280, la bascule s'étirait en deux boutons de 580 dp pour deux mots, et le calendrier en cases de 165 dp pour deux chiffres. Bornées mais avec une marge différente, les deux boîtes ne s'alignaient pas et le décalage se voyait. |
| Filet du jour courant | « filet `primary` 2 dp sur le bord gauche », compris comme une bordure | un `ColoredBox` de 2 dp **posé par-dessus** le bloc, comme `DayCell` au ticket 011 | Flutter refuse un `Border` non uniforme sous un `borderRadius`. Le ticket 011 avait déjà tranché de la même façon ; le brief ne le savait pas. |
| Borne de l'historique | « `date` ≥ il y a douze mois » | filtre `gte` sur **`shifts.date`**, la table jointe | `assignments` ne porte aucune date de créneau : la seule date qu'elle connaît est `proposed_at`, celle de la publication. Borner dessus aurait coupé un planning publié tôt pour un mois lointain. |
| Bandeau de fraîcheur en ligne | « Ces astreintes datent du 12 octobre. » | **deux phrases, une par cas** : hors ligne, « Hors ligne. » + « Dernière mise à jour : il y a 2 h. » ; en ligne après un échec, « Ces astreintes n'ont pas pu être actualisées. » + la même seconde ligne, plus « Réessayer » | Le deux-points tient aussi bien un relatif (« il y a 2 h », « hier ») qu'une date (« 15 sept. »), là où « ces astreintes datent hier » ne se lit pas. Et `formaterInstantRelatif` fait déjà la bascule relatif/absolu : deux formulations de l'ancienneté dans une même application en feraient deux vocabulaires. |
| Encre de la marque du calendrier | « case pleine à l'encre d'« accepté » (`#1A6B3A`) » — deux valeurs qui ne vont pas ensemble | la marque prend l'encre de **`etat-disponible-plein`**, lue à travers le thème, et porte l'**icône du créneau** | `#1A6B3A` est `etat-disponible-plein`, pas l'encre d'« accepté » (`#CFE9D8`, vert pâle). Le pâle ne tient pas en niveaux de gris (0,78 contre 1,00 pour une case vide), et c'est justement la lecture signature de l'écran. L'encre est donc prise pour sa **valeur**, pas pour son sens : sur cet écran « accepté » est universel et ne distingue rien, c'est le créneau qui porte l'information. |
| Raison d'une flèche de mois désactivée | « désactivée avec sa raison, pas cachée » | la raison est le **nom accessible** de la flèche et son info-bulle, pas une phrase posée à côté | Une phrase permanente sous la barre de mois coûterait 20 dp de grille pour redire ce qu'une flèche grise dit déjà. Le nom porte la raison : un lecteur d'écran annonce « Tu n'as pas d'astreinte avant ce mois, bouton, désactivé ». |
| Astreintes d'un planning archivé | non traité | elles **disparaissent** de la liste à la première lecture réussie qui suit l'archivage | Ni `assignments_select_own_published` ni `shifts_select_member_published` ne laissent sortir un planning `archived` : la base ne les rend plus, et le cache est remplacé en bloc à chaque lecture réussie. L'application dit donc ce que la base dit. À revoir si l'archivage mensuel se met à tourner (ticket 022). |
| Déconnexion et caches | non traité : le § 8.2 ne parlait que d'écriture et de relecture | les deux caches exposent un **effacement**, appelé par `DeconnexionController` **avant** la fermeture de session | Relevé en revue, avec un stockage réel : après déconnexion, le nom de la caserne et surtout **les noms des autres membres du créneau** restaient sur l'appareil. C'est exactement la règle que le ticket 024 s'était donnée pour la destination en attente, appliquée à ce qu'elle visait vraiment. Avant la fermeture, parce qu'après, la clé n'est plus composable. |
| Portée du repli sur le cache d'appartenances | « une lecture qui échoue retombe sur ce qui est gardé » | **seuls les échecs de transport** (`AuthErreur.reseau`) retombent ; un refus, un jeton périmé, une réponse illisible remontent tels quels | Un membre retiré de sa caserne aurait continué de la voir, masqué par un instantané périmé. `SupabaseMembershipRepository` distingue désormais une `PostgrestException` sans code — une requête jamais partie — du reste. |
| Rétrogradation du rôle | posée dans la relecture du document | posée **dans `appartenancesProvider`** (`Appartenance.commeMembre`), le codec ne faisant plus que la seconde ceinture | La garantie ne doit pas dépendre de l'implémentation de stockage branchée : la mémoire de test contournait la relecture et rendait un admin. Le test qui le prouve existe maintenant. |
| Mémoires de test | non traité | les deux rangent par les **mêmes clés que les vraies** — caserne et membre | Une mémoire qui rend le même contenu à n'importe qui ne peut attraper aucune régression de cloisonnement, et c'est précisément ce qu'on vérifie. |
| État vide et repli des passées | « l'état vide » et « le repli » comme deux états exclusifs | quand rien n'est à venir **mais** qu'il y a un passé, l'état vide se pose **au-dessus** du repli | Un « Astreintes passées (14) » seul au milieu d'un écran ne dit pas qu'il n'y a rien à venir ; il faut le lire par déduction. |
