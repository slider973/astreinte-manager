...
> est la référence, pas ce qui précède. Les trois premiers sont des défauts trouvés à l'écran, pas
> sur le papier.

| Point | Ce que disait ce brief | Ce que fait le code | Pourquoi |
|---|---|---|---|
| Position de la rangée d'actions | « sur leur propre rangée, pleine largeur de la ligne », comprise comme alignée sur le corps | la rangée prend **toute la largeur de la ligne, marge du registre comprise** | Alignée sur le corps, elle perdait les 60 dp de la marge : à 390 dp, « Refuser » s'écrivait « Ref… ». Un libellé d'action tronqué est un défaut, pas un compromis. Vu dans Chrome, capture à l'appui. |
| Boutons de la feuille de refus | « Deux boutons : "Garder le créneau", "Refuser" », côte à côte | **empilés en `compact`**, côte à côte au-delà. Et l'ordre est inversé : « Refuser le créneau » en haut, « Garder le créneau » en bas | Côte à côte à 390 dp, « Garder le créneau » s'écrivait « Garder le crén… », et raccourcir le libellé aurait coûté ce qu'il nomme. Empilés, la sortie **inoffensive** est la plus proche du pouce : un refus est irréversible, il ne doit pas être le bouton qu'on atteint sans viser. |
| Phrase de l'attribution disparue | une phrase longue en ligne principale de bannière | la bannière porte **« Ce créneau ne t'est plus proposé. »** en première ligne et le créneau + la raison en seconde | `AppBanner` borne chaque ligne à deux lignes de texte : la phrase d'origine s'ellipsait à « … confié à qu… ». Le créneau passe en tête du détail — c'est ce qu'on cherche en lisant. |
| Largeur des actions en `expanded` | « les deux boutons remontent à droite du corps » dès `expanded` | seuil mesuré à **680 dp de largeur de liste** (`largeurActionsACote`), rangée de **360 dp** | Un seuil exprimé en classe de fenêtre ignorait la colonne de 720 dp et le rail de navigation. Le seuil porte sur la largeur **réellement** disponible, lue par un `LayoutBuilder`. |
| Ordre rendu par PostgREST | non traité | le tri est fait en Dart, et **c'était nécessaire** : la base a rendu 19, 24, 12 pour trois attributions publiées dans la même transaction | Mesuré sur la pile locale. Sans tri client, la liste aurait été dans l'ordre physique des lignes. |
| Bannière de validation du planning | « le libellé du mois pris du planning relu » | le mois vient du **créneau répondu** | Un planning couvre exactement un mois (`docs/SCHEMA.md § 2.5`, `§ 2.9`) : le mois du créneau **est** celui du planning, et la relecture ne rend que le statut. Une requête de moins. |
| Rafraîchissement au retour au premier plan | annoncé au § 7.3 | livré via `WidgetsBindingObserver` **dans l'écran**, pas dans le contrôleur | Le contrôleur vit sur tous les onglets : y poser l'observateur aurait relu la liste au retour de la matrice admin, pour une pastille qui n'a pas bougé. |
| `PrimaryButton` | composant figé du socle | deux paramètres ajoutés : `libelleAnnonce` (le lecteur d'écran dit « Accepter samedi 12 octobre, nuit ») et `raisonVisible` (le second de deux boutons appairés n'écrit pas la raison déjà écrite par le premier) | Quatre boutons « Accepter » sur un écran ne se distinguent pas à l'oreille, et deux raisons de désactivation identiques sous une même ligne sont du bruit. L'invariant tient : la raison reste obligatoire et reste annoncée. |
| Onglet demandé par l'URL | le lien `/proposals` « mène déjà » à l'onglet 1 | **il n'y menait pas** : `AccueilScreen` ne suivait pas `ongletInitial` après le premier montage. `didUpdateWidget` le corrige | `go` change la chaîne de requête mais réutilise le même `State`. Le lien d'une notification ramenait sur « Mon mois » dès que l'application était déjà ouverte — exactement le chemin que ce ticket promet en deux touches. Trouvé par le test, confirmé dans Chrome. |
| `PlanningSql` | non prévu | extension ajoutée à `core/supabase/enums.dart`, et `PlanningBrouillon` l'utilise désormais | `schedule_status` se lisait dans une fonction privée de la fonctionnalité `planning`, qu'une autre fonctionnalité n'a pas le droit d'importer. Le duplicat aurait divergé. |
